...
std::vector<std::vector<int>> parse_puzzle(const std::string& filename) {
    ...
    for (int i = 0; i < N; ++i) {
        ...
        while (iss >> num) {
            if (j >= N) {
                throw std::invalid_argument("Too many numbers in row " + std::to_string(i + 1));
            }

            if (num < 0 || num >= N * N) {
                throw std::invalid_argument("Number out of range in row " + std::to_string(i + 1));
            }
        ...
        if (j != N) {
            throw std::invalid_argument("Too few numbers in row " + std::to_string(i + 1));
        }
    }
